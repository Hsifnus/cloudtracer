...
int main( int argc, char** argv ) {
  ...
  while ( (opt = getopt(argc, argv, "s:l:t:m:e:h:H:f:r:c:a:p:b:d:M:D:P:")) != -1 ) {  // for each option...
    switch ( opt ) {
      case 'f':
          write_to_file = true;
      ...
      case 'a':
          ...
          break;
      case 'M':
          config.pathtracer_mode = atoi(optarg);
          break;
      case 'D':
          config.pathtracer_deltaCeiling = atof(optarg);
          break;
      case 'H':
